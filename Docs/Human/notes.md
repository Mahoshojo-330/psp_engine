...
char list[0x20000] __attribute__((aligned(64))); // where the CPU writes draw commands for GPU
...
Texture * loadTexture(const char * filename) {
    ...
    sceKernelDcacheWritebackInvalidateAll(); // what's this for?
...
void drawTexture(Texture * texture, float x, float y, float w, float h) {
    ...
    vertices[0].u = 0.0f;
    vertices[0].v = 0.0f;
    vertices[0].colour = 0xFFFFFFFF; // can I remove this?
    vertices[0].x = x;
    vertices[0].y = y;
    vertices[0].z = 0.0f;
    ...
    vertices[1].x = x + w;
    vertices[1].y = y + h;
    vertices[1].z = 0.0f; //depth? this is 2d

    sceGuTexMode(GU_PSM_8888, 0, 0, GU_FALSE);
    sceGuTexFunc(GU_TFX_REPLACE, GU_TCC_RGB); // WHAT?
    sceGuTexImage(0, texture->width, texture->height, texture->width, texture->data);

    sceGuEnable(GU_TEXTURE_2D);  // 2D texture
